...
recurrent_map = {
    'valid': RecurrentMode.valid,
    'full': RecurrentMode.full,
}


def __get_recurrent_mode_string(mode):
    if isinstance(mode, str):
        if mode in recurrent_map:
            return mode
        else:
            raise ValueError('No such recurrent mode: {}'.format(mode))
    elif isinstance(mode, RecurrentMode):
        return mode.name
    else:
        raise ValueError('Invalid mode type: {}'.format(type(mode)))


def basic_recurrent_shape(shape_data, shape_kernel, dimension: int, mode: str):
    if mode == 'valid':
